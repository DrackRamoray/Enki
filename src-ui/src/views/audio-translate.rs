...
use crate::stores::audio_translate_ctx::use_audio_translate_ctx;
use crate::stores::audio_translate_pf_ctx::use_audio_translate_pf_ctx;
...
#[component]
pub fn AudioTranslate(cx: Scope) -> impl IntoView {
    let app_ctx = use_context::<AppCtx>(cx);
    let app_ctx = app_ctx.unwrap_or_else(|| use_app_ctx(cx));

    if app_ctx.active_topic_ctg.get() != TopicCategory::AudioTranslate.to_index() {
        return view! { cx, <div></div> };
    }

    let parent_ref = create_node_ref(cx);
    let ctx = use_audio_translate_ctx(cx);
    ...
    let send = create_action(cx, move |_: &()| async move {
        let pf_ctx = use_audio_translate_pf_ctx(cx, app_ctx.active_topic_id, on_finished);
    ...
    let fetcher = create_resource(
        ...
        move |_| async move {
            ...
            match tauri::invoke::<_, Vec<AudioData>>(
                "plugin:audio_translate|get_audio_translate_record",
                &params,
            )
            .await
            {
